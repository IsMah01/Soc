...
KIBANA_AUTH="elastic:<REMOVED_DEFAULT_PASSWORD>"
...
# Kibana connector
echo -e "\nKibana Connector:"
curl -s -u elastic:<REMOVED_DEFAULT_PASSWORD> "http://localhost:5601/api/actions/connectors" \
  -H 'kbn-xsrf: true' | jq -r '.[] | select(.name | contains("TheHive")) | .name'

# Elastic alerts
echo -e "\nRecent Elastic Alerts:"
curl -s -u elastic:<REMOVED_DEFAULT_PASSWORD> "http://localhost:9200/.siem-signals*/_search?size=2" \
  -H 'Content-Type: application/json' \
...
echo "Username: elastic / <REMOVED_DEFAULT_PASSWORD>"
MONITOR
